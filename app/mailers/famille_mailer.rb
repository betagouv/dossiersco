class FamilleMailer < ApplicationMailer
  ...
  def emails
    @eleve.dossier_eleve.resp_legal.map{ |resp_legal| resp_legal.email } + ['contact@dossiersco.fr']
  end
  ...
  def message_differe(eleve, contenu)
    ...
    mail(subject: "Réinscription de votre enfant au collège", reply_to:['contact@dossiersco.fr', etablissement.email], to: ['contact@dossiersco.fr', resp_legal.email, etablissement.email]) do |format|
  ...
  def invitations_parents(eleve)
    ...
    mail(subject: "Réinscription de votre enfant au collège",
         reply_to: ['contact@dossiersco.fr', @eleve.dossier_eleve.etablissement.email], to: destinataires) do |format|
      format.text
    end
  end
